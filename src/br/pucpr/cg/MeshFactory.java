...
import br.pucpr.util.Noise;
import br.pucpr.util.TambosiNoise;
...
public class MeshFactory
{
    ...
    public static Mesh createTerrain(Shader shader, int width, int depth, int precision, float scale)
    {
        //var noise = new TambosiNoise(width, depth, 0.1f);
        ...
        for (int z = 0; z < depth; z++)
        {
            ...
            {
                var noise = TambosiNoise.Noise(x * precision, z * precision) ;

                positions.add(new Vector3f(x, (float)noise * scale, z).sub(offset));
            }
        }
    ...
    public static Mesh createTerrain(boolean withBaseNoiseShader, Shader shader, int width, int depth, float precision, float scale)
    {
            var offset = new Vector3f(width / 2.0f, 0.0f, depth / 2.0f);
            var positions = new ArrayList<Vector3f>();
            for (int z = 0; z < depth; z++)
            {
                for (int x = 0; x < width; x++)
                {
                    var tone = Noise.noise(x * precision, z * precision, precision);

                    positions.add(new Vector3f(x, (float)tone * scale, z).sub(offset));
                }
            }
